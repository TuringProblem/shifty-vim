	...
	for _, item in ipairs(items) do
		...
		if vim.fn.isdirectory(item_path) == 1 then
			...
			if vim.fn.filereadable(init_file) == 1 then
				local language_info = M.load_language_module(item, "shifty.languages", config)
...
function M.auto_register_builtin_languages(config)
	...
	for _, language in ipairs(builtin_languages) do
		local success, _ = pcall(function()
			return M.load_language_module(language, "shifty.languages", config)
		end)
